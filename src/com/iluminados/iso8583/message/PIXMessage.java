...
public class PIXMessage extends AbstractISO8583Message {
	
	static final Logger log = Logger.getLogger("PIXMessage");
	
	public static final String OPERATION_MTI = "0100";
	public static final String PROCESSING_CODE = "400050";
	public static final String DEFAULT_OPER_NII= "5";

	@Getter String idOperacao;
	
	
	String valor;
	public PIXMessage(String valor) {
		this.valor = valor;
	}
	...
	private void handleResponse(byte[] responseBytes) throws ISOException, IOException {
		...
		if (respCode.equals("00")) {
			...
			for (SubField sf : list) {
				...
				switch (sf.getId()) {
				case "10":
						idOperacao = new String(data);
						log.info("ID Operação : " + idOperacao);
					break;
				case "20":
						byte [] qrcode = Base64.getDecoder().decode(data);
						FileOutputStream out = new FileOutputStream(new File("data/qrcode.png"));
						out.write(qrcode);
						out.close();
					break;
				}
